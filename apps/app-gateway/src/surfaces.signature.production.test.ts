...
const makeSignedBundleFile = (registry: SurfaceRegistry, mutate?: (r: SurfaceRegistry) => SurfaceRegistry) => {
...
const setupProdPublicEnv = () => {
  ...
  process.env.APP_GATEWAY_PUBLIC_BASE_URL = process.env.APP_GATEWAY_PUBLIC_BASE_URL ?? "https://gateway.example";
...
await run("production startup fails closed on tampered surface bundle (gateway)", async () => {
  ...
  const { bundlePath, publicKeyEnv } = (() => {
    ...
    parsed.registry.services = Array.isArray(parsed.registry.services) ? parsed.registry.services.slice(0, 1) : [];
  ...
  const app = buildServer({ configOverride: { ...config } });
  try {
    await assert.rejects(async () => app.ready(), (e: unknown) => {
