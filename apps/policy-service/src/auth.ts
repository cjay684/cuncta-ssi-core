import { FastifyReply, FastifyRequest } from "fastify";
import { extractBearerToken, makeErrorResponse, verifyServiceJwt } from "@cuncta/shared";
import { config } from "./config.js";

export const requireServiceAuth = async (
  request: FastifyRequest,
  reply: FastifyReply,
  options?: { requiredScopes?: string[] }
) => {
  const serviceSecret =
    config.SERVICE_JWT_SECRET_POLICY ??
    (config.ALLOW_LEGACY_SERVICE_JWT_SECRET ? config.SERVICE_JWT_SECRET : undefined);
  const nextSecret = config.SERVICE_JWT_SECRET_NEXT;
  if (!serviceSecret) {
    return reply.code(503).send(
      makeErrorResponse("service_auth_not_configured", "Service authentication is not configured", {
        devMode: config.DEV_MODE
      })
    );
  }
  const token = extractBearerToken(request.headers.authorization);
  if (!token) {
    return reply.code(401).send(
      makeErrorResponse("invalid_request", "Missing service token", {
        devMode: config.DEV_MODE
      })
    );
  }
  try {
    const audience = config.SERVICE_JWT_AUDIENCE_POLICY ?? config.SERVICE_JWT_AUDIENCE;
    try {
      await verifyServiceJwt(token, {
        audience,
        secret: serviceSecret,
        issuer: "app-gateway",
        subject: "app-gateway",
        requiredScopes: options?.requiredScopes
      });
    } catch (error) {
      if (!(error instanceof Error) || error.message !== "jwt_missing_required_scope") {
        if (!nextSecret) throw error;
        await verifyServiceJwt(token, {
          audience,
          secret: nextSecret,
          issuer: "app-gateway",
          subject: "app-gateway",
          requiredScopes: options?.requiredScopes
        });
      } else {
        throw error;
      }
    }
  } catch (error) {
    if (error instanceof Error && error.message === "jwt_missing_required_scope") {
      return reply.code(403).send(
        makeErrorResponse("service_auth_scope_missing", "Service token scope missing", {
          devMode: config.DEV_MODE
        })
      );
    }
    return reply.code(401).send(
      makeErrorResponse("invalid_request", "Invalid service token", {
        devMode: config.DEV_MODE
      })
    );
  }
};
