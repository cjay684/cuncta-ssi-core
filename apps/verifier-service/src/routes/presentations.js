...
const getActionForPolicyId = (policyId) => {
  const map = policyMap;
  return map[policyId] ?? null;
};
const fetchRequirements = async (action) => {
...
const evaluatePredicate = (predicate, claims) => {
...
export const registerPresentationRoutes = (app) => {
  app.post("/v1/presentations/request", async (request, reply) => {
    ...
    if (!action) {
      return reply.code(400).send({ error: "policy_id_unknown" });
    }
    let requirements;
    try {
      requirements = await fetchRequirements(action);
    } catch (error) {
      log.error("presentation.request.policy_failed", { error, action });
      return reply.code(502).send({ error: "policy_unavailable" });
    }
  ...
  app.post("/v1/presentations/verify", async (request, reply) => {
    ...
    const presentation =
      "presentation" in body && typeof body.presentation === "string"
        ? body.presentation
        : body.sdJwtPresentation;
    const tokenHash = sha256Hex(presentation);
    log.info("presentation.verify", { tokenHash });
    try {
      ...
      if (matchingRequirement.revocation?.required ?? true) {
        const statusCheck = await verifyStatus(status);
      ...
      log.error("presentation.verify.failed", { error });
      return reply.send({
        valid: false,
        claims: {},
