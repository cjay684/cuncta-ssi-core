...
const run = async () => {
  ...
  try {
    ...
    const env = {
      ...
      CAPABILITY_WORKER_POLL_MS: "999999",
      ...
      RETENTION_CAPABILITY_SIGNALS_DAYS: "999",
    ...
    assert.equal(parsed.CAPABILITY_WORKER_POLL_MS, 30_000);
    ...
    assert.equal(parsed.RETENTION_CAPABILITY_SIGNALS_DAYS, 365);
