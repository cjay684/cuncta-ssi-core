...
export const vpRespond = async () => {
  ...
  let checkpoint = "init";
  const logStage = (substage: string, event: "start" | "done", detail?: Record<string, unknown>) => {
    ...
    console.log(`stage=vp.respond|substage=${substage}|event=${event}|payload=${JSON.stringify(payload)}`);
  };
  ...
  logStage("request.parse_input", "start");
    const requestInput = await withStageTimeout("request.resolve_arg", async () => resolveRequestArg(args.request));
    const requestObj =
      requestInput.trim().startsWith("http://") || requestInput.trim().startsWith("https://")
        ? await withStageTimeout("request.fetch", async () => fetchJson<unknown>(requestInput.trim()))
        : (JSON.parse(requestInput) as unknown);
    logStage("request.parse_input", "done");

    let req: z.infer<typeof Oid4vpRequestObjectSchema>;
    logStage("request.schema_validate", "start");
    req = Oid4vpRequestObjectSchema.parse(requestObj);
  ...
  const strictRequestSignature =
    ...
    const requestJwt = req.request_uri
      ? (await withStageTimeout("request.fetch_jwt_uri", async () => fetchText(String(req.request_uri)))).trim()
      : (req.request_jwt ?? "");
  ...
  const canonicalNonce = typeof requestJwtPayload.nonce === "string" ? requestJwtPayload.nonce : req.nonce;
  ...
  const stateValue = typeof requestJwtPayload.state === "string" ? requestJwtPayload.state : req.state;
  ...
  for (const predicate of requirement?.predicates ?? []) {
    ...
    const requirementExtras = requirement as unknown as { formats?: unknown; zk_predicates?: unknown };
  const allowedFormats = Array.isArray(requirementExtras.formats) ? requirementExtras.formats.map(String) : ["dc+sd-jwt"];
  ...
  const hasDi = (state.credentials ?? []).some(
    (c: { vct?: unknown; format?: unknown }) => String(c.vct ?? "") === vct && String(c.format ?? "") === "di+bbs"
  );
  ...
  const chosenFormat: "dc+sd-jwt" | "di+bbs" = preferDi
    ...
    const zkPredicatesRaw = Array.isArray(requirementExtras.zk_predicates) ? requirementExtras.zk_predicates : [];
    ...
    for (const p of zkPredicatesRaw) {
      ...
      if (statement.definition.credential.format !== "dc+sd-jwt") {
        throw new Error(`zk_credential_format_unsupported:${statement.definition.credential.format}`);
      }
    ...
    if (!credential || typeof credential !== "string") throw new Error(`credential_missing:${vct}`);
    logStage("credential.select_sdjwt", "done", { hasZkPredicates: resolvedZkPredicates.length > 0 });
    ...
    if (resolvedZkPredicates.length > 0) {
      const requestZkContext = (requestJwtPayload.zk_context as Record<string, unknown> | undefined) ?? {};
      ...
      for (const item of resolvedZkPredicates) {
        ...
        const stateWithSecrets = state as unknown as { zkSecrets?: Record<string, Record<string, unknown>> };
        ...
        const witnessBuilder = getWalletWitnessBuilder(statement.definition.wallet_contract.witness_builder_id);
        if (!witnessBuilder) {
          throw new Error(`zk_witness_builder_missing:${statement.definition.wallet_contract.witness_builder_id}`);
        }
        ...
        proofs.push({
          ...
          bindings: { nonce: canonicalNonce, audience: canonicalAudience, request_hash: requestHash }
        });
      }
      zkProofs = proofs;
    ...
    const fromEntry =
      entry && typeof (entry as any).issuerBbsPublicKeyB64u === "string"
        ? String((entry as any).issuerBbsPublicKeyB64u)
        : "";
  ...
  // We sign a response JWT with the holder key and POST it as form-encoded `response=...`.
    const pd = presentationDefinition as unknown as { input_descriptors?: Array<{ id?: unknown }>; id?: unknown };
    const inputDescriptorId = String(pd?.input_descriptors?.[0]?.id ?? req.requirements[0]?.vct ?? vct);
    const submission = {
    id: `ps_${randomUUID()}`,
  ...
  logStage("response.post", "start", { responseUri });
    ...
    const verifyRecord =
      verify && typeof verify === "object" ? (verify as Record<string, unknown>) : ({} as Record<string, unknown>);
    console.log(
      JSON.stringify({
