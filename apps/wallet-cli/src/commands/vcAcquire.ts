...
const withHeartbeat = async <T>(stage: string, fn: () => Promise<T>): Promise<T> => {
  ...
  const timer = setInterval(() => {
    console.log(`stage=vc.acquire|substage=${stage}|event=heartbeat|elapsedMs=${Date.now() - started}`);
  }, 5000);
  timer.unref?.();
  try {
    return await fn();
...
export const vcAcquire = async () => {
  ...
  const logStage = (substage: string, event: "start" | "done") => {
    console.log(`stage=vc.acquire|substage=${substage}|event=${event}|elapsedMs=${Date.now() - commandStarted}`);
  };
  ...
  const claimsForOffer: Record<string, unknown> = claimsJsonInput && claimsJsonInput.trim().length
    ? (() => {
        try {
  ...
  const offerObj = await withHeartbeat("offer.resolve", async () => {
    ...
    if (args.configId.startsWith("aura:")) {
      ...
      const spaceId = typeof claimsForOffer.space_id === "string" ? claimsForOffer.space_id : undefined;
      if (!domain && !spaceId) {
  ...
  const preauthGrant = parsedOffer.grants["urn:ietf:params:oauth:grant-type:pre-authorized_code"] as
    | Record<string, unknown>
    | undefined;
  const preAuthorizedCode = typeof preauthGrant?.["pre-authorized_code"] === "string"
  ...
  const resolved =
    String(configId).startsWith("sdjwt:")
  ...
  if (resolved.vct === "age_credential_v1") {
    ...
    if (!Number.isFinite(birthdateDays)) {
      throw new Error("WALLET_BIRTHDATE_DAYS required for age_credential_v1 issuance (days since epoch)");
    }
  ...
  (state as unknown as { credentials?: StoredCredential[] }).credentials = next;
    ...
    console.log(JSON.stringify({ ok: true, vct: resolved.vct, format: resolved.format, configId }, null, 2));
  } finally {
