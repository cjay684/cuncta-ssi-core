...
export const ZkStatementDefinitionSchema = z.object({
  ...
  .refine((def) => def.verifier_contract.required_public_inputs.join(",") === def.public_inputs_order.join(","), {
    message: "verifier_contract.required_public_inputs must match public_inputs_order"
  })
  .refine((def) => def.public_inputs_schema.required.every((k) => def.public_inputs_order.includes(k)), {
